<?php
/**
 * Data Collector - Add Property Form with Offline Sync
 * properties/add.php
 */
...
// Enhanced validation function for properties
function validatePropertyData($data, $db) {
    $errors = [];
    ...
    if (empty(trim($data['structure'] ?? ''))) {
        $errors[] = "Structure is required.";
    } elseif (strlen(trim($data['structure'])) > 100) {
        $errors[] = "Structure must be 100 characters or less.";
    }
    
    if (empty(trim($data['property_use'] ?? ''))) {
        $errors[] = "Property use is required.";
    }
    
    // Optional field length validation
    if (!empty($data['telephone']) && strlen(trim($data['telephone'])) > 20) {
        $errors[] = "Telephone must be 20 characters or less.";
    }
    ...
    $number_of_rooms = intval($data['number_of_rooms'] ?? 0);
    if ($number_of_rooms <= 0) {
        $errors[] = "Number of rooms must be greater than 0.";
    } elseif ($number_of_rooms > 50) {
        $errors[] = "Number of rooms cannot exceed 50.";
    }
...
function sanitizePropertyData($data) {
    return [
        ...
        'structure' => substr(trim($data['structure'] ?? ''), 0, 100),
        'ownership_type' => in_array($data['ownership_type'] ?? '', ['Self', 'Family', 'Corporate', 'Others']) ? $data['ownership_type'] : 'Self',
        'property_type' => in_array($data['property_type'] ?? '', ['Modern', 'Traditional']) ? $data['property_type'] : 'Modern',
        'number_of_rooms' => max(1, min(50, intval($data['number_of_rooms'] ?? 1))),
        'property_use' => in_array($data['property_use'] ?? '', ['Commercial', 'Residential']) ? $data['property_use'] : 'Residential',
...
if (isset($_GET['action']) && $_GET['action'] === 'sync_offline_data') {
    ...
    if (!$input || !isset($input['properties']) || !is_array($input['properties'])) {
    ...
    try {
        ...
        foreach ($input['properties'] as $propertyData) {
            try {
                // Enhanced validation
                $validationErrors = validatePropertyData($propertyData, $db);
                ...
                // Sanitize data
                $cleanData = sanitizePropertyData($propertyData);
                
                // Get fee per room
                $fee_check = $db->fetchRow(
                    "SELECT fee_per_room FROM property_fee_structure 
                     WHERE structure = ? AND property_use = ? AND is_active = 1",
                    [$cleanData['structure'], $cleanData['property_use']]
                );
                
                if (!$fee_check) {
                    throw new Exception('Invalid structure and property use combination');
                }
                
                // Calculate current bill
                $current_bill = $fee_check['fee_per_room'] * $cleanData['number_of_rooms'];
                
                // Calculate amount payable
                $amount_payable = max(0, $cleanData['old_bill'] - $cleanData['previous_payments']) + $current_bill;
                ...
                try {
                    ...
                    $params = [
                        ...
                        $cleanData['longitude'],
                        $cleanData['structure'],
                        $cleanData['ownership_type'],
                        $cleanData['property_type'],
                        $cleanData['number_of_rooms'],
                        $cleanData['property_use'],
                        $cleanData['old_bill'],
                        ...
                        if ($property_id) {
                            ...
                            $db->execute(
                                "INSERT INTO audit_logs (user_id, action, table_name, record_id, new_values, ip_address, user_agent, created_at)
                                 VALUES (?, 'CREATE_PROPERTY_SYNC', 'properties', ?, ?, ?, ?, NOW())",
                                [
                                    $currentUser['user_id'], 
                                    $property_id,
                                    json_encode([
                            ...
                            throw new Exception('Failed to get property ID after insert');
                        }
                    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !isset($_GET['action'])) {
    try {
        ...
        // Enhanced validation
        $validationErrors = validatePropertyData($_POST, $db);
        
        if (!empty($validationErrors)) {
            $errors = $validationErrors;
        } else {
            // Sanitize data
            $cleanData = sanitizePropertyData($_POST);
                ...
                try {
                    ...
                    if ($stmt) {
                        ...
                        if ($property_id) {
                            ...
                            throw new Exception("Failed to get property ID");
                        }
                    } else {
                        throw new Exception("Failed to register property");
                    }
...
<body>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="owner_name" class="form-control" maxlength="100"
                                       ...
                                       required>
                                <div class="form-help">Enter the full name of the property owner (max 100 characters)</div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-help">Contact number for the property owner (max 20 characters)</div>
                            </div>
                        </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <div class="form-help">Total number of rooms in the property (1-50)</div>
                            </div>
    ...
    <script>
        // === ENHANCED OFFLINE SYNC SYSTEM FOR PROPERTIES ===
        
        // Global variables for offline sync
        let isOnline = navigator.onLine;
        let syncInProgress = false;
        
        // IndexedDB setup for property offline storage
        let db;
        const dbName = 'PropertyFormDB';
        const dbVersion = 1;
        
        // Enhanced IndexedDB initialization
        function initDB() {
            return new Promise((resolve, reject) => {
                console.log('Initializing Property IndexedDB...');
                ...
                request.onsuccess = (event) => {
                    ...
                    console.log('Property IndexedDB opened successfully');
                ...
                request.onupgradeneeded = (event) => {
                    console.log('Upgrading Property IndexedDB schema...');
                    db = event.target.result;
                    ...
                    try {
                        ...
                        const store = db.createObjectStore('propertyForms', { 
                            keyPath: 'id',
                            autoIncrement: true
                        });
                        ...
                        store.createIndex('owner_name', 'owner_name', { unique: false });
                        
                        console.log('Property IndexedDB schema created successfully');
                    } catch (error) {
        ...
        function generateClientId() {
            ...
            return `prop_client_${timestamp}_${randomStr}${moreRandom}`;
        }
        
        // Enhanced data validation with schema constraints
        function validatePropertyDataClient(formData) {
            const errors = [];
            ...
            if (!formData.structure || formData.structure.trim().length === 0) {
                errors.push('Structure is required');
            } else if (formData.structure.length > 100) {
                errors.push('Structure must be 100 characters or less');
            }
            
            if (!formData.property_use || formData.property_use.trim().length === 0) {
            ...
            const number_of_rooms = parseInt(formData.number_of_rooms);
            if (!number_of_rooms || number_of_rooms <= 0) {
                errors.push('Number of rooms must be greater than 0');
            } else if (number_of_rooms > 50) {
                errors.push('Number of rooms cannot exceed 50');
            }
        ...
        // Sanitize property data for storage with proper constraints
        function sanitizePropertyData(formData) {
            const sanitized = {};
        ...
        function sanitizeValue(key, value) {
            ...
            // Apply length limits and type conversions based on schema
            switch (key) {
                case 'owner_name':
                ...
                case 'batch':
                    ...
                    break;
                case 'gender':
                    if (!['Male', 'Female', 'Other'].includes(sanitized)) return null;
                    break;
                case 'ownership_type':
                    if (!['Self', 'Family', 'Corporate', 'Others'].includes(sanitized)) return 'Self';
                    break;
                case 'property_type':
                    if (!['Modern', 'Traditional'].includes(sanitized)) return 'Modern';
                    break;
                case 'property_use':
                    if (!['Commercial', 'Residential'].includes(sanitized)) return 'Residential';
                    break;
                case 'latitude':
                case 'longitude':
                ...
                case 'number_of_rooms':
                    ...
                    if (isNaN(id) || id <= 0) return key === 'number_of_rooms' ? 1 : null;
                    return id;
                default:
                    return sanitized;
            }
        ...
        async function saveToIndexedDB(formData) {
            try {
                ...
                const validationErrors = validatePropertyDataClient(sanitizedData);
                ...
                return new Promise((resolve, reject) => {
                    const transaction = db.transaction(['propertyForms'], 'readwrite');
                    const store = transaction.objectStore('propertyForms');
                    
                    transaction.oncomplete = () => {
                        console.log('Property data saved to IndexedDB successfully:', sanitizedData.client_id);
        ...
        async function getUnsyncedForms() {
            try {
                ...
                return new Promise((resolve, reject) => {
                    const transaction = db.transaction(['propertyForms'], 'readonly');
                    const store = transaction.objectStore('propertyForms');
                    const results = [];
                    ...
                    request.onsuccess = (event) => {
                        ...
                        if (cursor) {
                            ...
                            // No more entries
                            console.log(`Found ${results.length} unsynced property forms`);
                            resolve(results);
                        }
                    };
        ...
        async function syncOfflineData() {
            ...
            try {
                ...
                if (unsyncedForms.length === 0) {
                    console.log('No offline property data to sync');
                    return;
                }
                
                console.log(`Syncing ${unsyncedForms.length} offline property forms...`);
                ...
                if (result.success) {
                    ...
                    console.log(`Marked ${syncedCount} property forms as synced locally`);
                } else {
        ...
        async function showSyncStatus() {
            try {
                ...
                const message = unsyncedForms.length === 0 
                    ? 'All data is synced!' 
                    : `${unsyncedForms.length} property forms pending sync`;
        ...
        async function handleFormSubmission(formData) {
            ...
            try {
                ...
                showSyncMessage('info', 'Property data saved offline. Will sync when connection is restored.');
                ...
                showSyncMessage('error', 'Failed to save property data: ' + error.message);
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            try {
                ...
                while (retryCount < maxRetries) {
                    try {
                        await initDB();
                        console.log('Property IndexedDB initialized successfully');
                        break;
                    } catch (error) {
                        retryCount++;
                        console.warn(`Property IndexedDB init attempt ${retryCount} failed:`, error);
                        
                        if (retryCount >= maxRetries) {
                            console.error('Failed to initialize Property IndexedDB after', maxRetries, 'attempts');
                ...
                console.log('Property offline sync system initialized successfully');
                
            } catch (error) {
                console.error('Failed to initialize property offline sync system:', error);
